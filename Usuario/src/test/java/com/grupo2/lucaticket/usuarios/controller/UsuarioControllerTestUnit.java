...
import com.grupo2.lucaticket.usuario.UsuarioApplication;
...
@ContextConfiguration(classes={UsuarioApplication.class})
...
List<Usuario> usuarioVacio;

private final int ID = 1;
private final int ID_NULL = (Integer);
...
public void cuandoListaNull_Devuelve404() throws Exception {
	...
	// then
	mockMvc.perform(get("/usuarios").contentType("application/json")).andExpect(status().isNotFound());
...
public void cuandoBorroUsuario_daOk() throws Exception{
	...
	mockMvc.perform(delete("/usuario/" + usuario.getId()).contentType("application/json"))
...
public void cuandoUsuarioPorId_Devuelve200() throws Exception {
	...
	// then
	mockMvc.perform(get("/usuario/id/" + usuarioDto.getId()).contentType("application/json"))
