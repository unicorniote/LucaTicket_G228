...
public class ValidarPagosController {
	...
	@Operation(summary = "Validar pago con tarjeta", description = "Dado un número de tarjeta y un código CVV se valida el pago", tags = {
			"student" })
	@ApiResponses(value = {
			@ApiResponse(responseCode = "203", description = "El CVV de la tarjeta no es válido", content = {
					@Content }),
			@ApiResponse(responseCode = "207", description = "La tarjeta no tiene fondos", content = @Content),
			@ApiResponse(responseCode = "404", description = "La tarjeta no existe en la base de datos", content = @Content) })
	@PostMapping("/pagar")
