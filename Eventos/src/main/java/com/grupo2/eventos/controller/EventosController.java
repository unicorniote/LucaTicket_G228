...
public class EventosController {
	...
	public ResponseEntity<?> addEvento(@Valid @RequestBody Evento evento) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(evento.getID())
				.toUri();
	...
	@Operation(summary = "Listar los eventos", description = "Lista todo los eventos existentes en la BBDD de MongoDB", tags = {
			"Evento" })

	@ApiResponses(value = {
			@ApiResponse(responseCode = "201", description = "Eventos encontrados", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = Evento.class)) }),
